...
class LampSettingsDialog(QDialog):
    ...
    def load_mode_settings(self):
        ...
        # Default mode configurations - updated for calm, fluid colors
        default_modes = {
            'productivity': {
                'primary_color': [72, 192, 255],   # #48C0FF - Calm productivity blue
                'movement': 10,                     # Very slow, calm movement
                'front_white': True,
                'white_temp': 4000
            },
            'creativity': {
                'primary_color': [129, 202, 95],   # #81CA5F - Natural creativity green
                'movement': 15,                     # Gentle, flowing movement
                'front_white': False,
                'white_temp': 4000
            },
            'social_media_detox': {
                'primary_color': [255, 205, 126],  # #FFCD7E - Warm detox orange
                'movement': 8,                      # Very slow, meditative movement
                'front_white': True,
                'white_temp': 2700
            }
        }
        ...
        # Check if creativity mode has the old purple color and reset if needed
        if ('creativity' in saved_settings and 
            saved_settings['creativity'].get('primary_color') == [96, 0, 255]):
            print("DEBUG: Detected old purple creativity color, clearing saved settings to use new defaults")
            saved_settings = {}
...
class Plugin(PluginBase):
    ...
    def on_session_start(self, session_data: Dict[str, Any]):
        ...
        print(f"DEBUG: Mode type: {type(mode)}, repr: {repr(mode)}")
    ...
    def set_lamp_mode(self, mode: str):
        ...
        # Check if creativity mode has the old purple color and reset if needed  
        if ('creativity' in mode_settings and 
            mode_settings['creativity'].get('primary_color') == [96, 0, 255]):
            print("DEBUG: Detected old purple creativity color in lamp mode, clearing saved settings")
            mode_settings = {}
            # Also clear from persistent storage
            self.set_setting('mode_settings', {})
        ...
        print(f"DEBUG: Final color being sent: RGB{settings['primary_color']}")
        print(f"DEBUG: Expected creativity color: RGB[129, 202, 95]")
        print(f"DEBUG: Expected detox color: RGB[255, 205, 126]")
