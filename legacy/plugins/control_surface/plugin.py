...
def end_session_event():
    ...
    try:
        ...
        # Get the control surface plugin instance
        if 'control_surface' in plugin_manager.loaded_plugins:
            plugin_instance = plugin_manager.loaded_plugins['control_surface']
            ...
            print("Control surface plugin not loaded")
...
class Plugin(PluginBase):
    ...
    def on_summary_closed(self, session_data):
        print("DEBUG: on_summary_closed hook called!")
        # Restore your original queue functionality
        queued = get_queued_mode()
        print(f"DEBUG: queued mode = {queued}")
        if queued: 
            print(f"DEBUG: Starting queued mode: {queued} (with 1 second delay)")
            # Add a small delay to ensure proper cleanup
            import time
            time.sleep(1)
            start_mode(queued) 
            set_queued_mode(None)
            print("DEBUG: Queued mode cleared")
        else:
            print("DEBUG: No queued mode to start")
