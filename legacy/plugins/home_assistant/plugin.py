...
class Plugin:
    ...
    def on_session_end(self, session_data: Dict[str, Any]):
        """Called when a focus session ends (normal or early termination)"""
        if not self.settings.get('enabled'):
            return
            
        # Reset to default regardless of session state for reliability
        self.reset_to_default("session ended")
    
    def on_goals_analyzed(self, goals, goals_text):
        """Hook called after goals are analyzed. Can modify goals list."""
        # Home Assistant doesn't need to modify goals, just return them as-is
        return goals
    
    def cleanup(self):
        """Called when plugin is disabled or app closes - ensure lights reset"""
        if not self.settings.get('enabled'):
            return
            
        print("Home Assistant: Plugin cleanup - resetting to default")
        self.reset_to_default("plugin cleanup")
    
    def reset_to_default(self, trigger_reason="unknown"):
        """Reset Home Assistant to default scene"""
        default_scene = self.settings['scenes'].get('default')
        if default_scene:
            print(f"Home Assistant: Returning to default scene ({trigger_reason})")
            success = self.activate_scene(default_scene)
            if success:
                print(f"Home Assistant environment restored to default ({trigger_reason})")
            else:
                print(f"Failed to restore Home Assistant to default ({trigger_reason})")
        else:
            print(f"No default scene configured - cannot reset ({trigger_reason})")
