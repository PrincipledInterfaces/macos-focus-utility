...
void updateAnimation() {
  ...
  // Dramatically increased animation speed
  animationPhase += 0.05 + (lampState.movement * 0.008);  // Was 0.08 + movement * 0.002, now much faster
...
CRGB getAnimatedColor(int ledIndex) {
  ...
  if (ledIndex == 0 && millis() % 3000 < 100) { // Reduce debug spam
    Serial.printf("DEBUG: Base RGB=(%d,%d,%d), HSV=(%d,%d,%d), movement=%d\n", 
                  baseRGB.r, baseRGB.g, baseRGB.b,
                  baseHSV.h, baseHSV.s, baseHSV.v, lampState.movement);
  }
  
  // Add position-based variation with multiple wave patterns
  float positionPhase = (float)ledIndex / NUM_LEDS * TWO_PI;
  float movementFactor = lampState.movement / 100.0;
  
  // DRAMATICALLY increased color variation - now should be very visible!
  float hueVariation = sin(animationPhase + positionPhase) * 60 * movementFactor;           // Was 25, now 60!
  hueVariation += sin(animationPhase * 1.7 + positionPhase * 0.5) * 30 * movementFactor;  // Additional hue wave
  
  float satVariation = sin(animationPhase * 1.3 + positionPhase) * 80 * movementFactor;    // Was 35, now 80!
  
  float valVariation = sin(animationPhase * 0.7 + positionPhase) * 120 * movementFactor;   // Was 50, now 120!
  valVariation += sin(animationPhase * 2.1 + positionPhase * 1.3) * 40 * movementFactor; // Additional brightness wave
  
  // Apply variations with wider ranges for more dramatic effects
  uint8_t newHue = baseHSV.hue + (int)hueVariation;
  uint8_t newSat = constrain(baseHSV.sat + (int)satVariation, max(100, baseHSV.sat - 100), 255); // Allow much lower sat
  uint8_t newVal = constrain(baseHSV.val + (int)valVariation, max(50, baseHSV.val - 150), 255);  // Allow much darker
  
  return CHSV(newHue, newSat, newVal);
}
